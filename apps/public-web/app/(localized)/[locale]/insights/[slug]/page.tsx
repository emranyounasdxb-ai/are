  ...
  return <div className="article-page" id="top"><main id="main-content">
    <article>
      ...
      <div className="article-layout"><aside className="article-toc"><strong>{ar ? "في هذا الدليل" : "In this guide"}</strong><nav aria-label={ar ? "محتويات المقال" : "Article contents"}>{copy.sections.map((section, index) => <a href={`#section-${index + 1}`} key={section.heading}>{section.heading}</a>)}<a href="#checklist">{copy.checklistTitle}</a><a href="#sources">{copy.sourcesTitle}</a></nav></aside><div className="article-body">
        ...
        <section className="article-sources" id="sources"><h2>{copy.sourcesTitle}</h2><ul>{article.sources.map((source) => <li key={source.url}><a href={source.url} rel="noreferrer" target="_blank">{source.name}<span aria-hidden="true">↗</span></a></li>)}</ul><p>{ar ? `تمت مراجعة هذه المراجع في ${article.updated}. أعد التحقق منها قبل الاعتماد على معلومات حالية.` : `These references were reviewed on ${article.updated}. Recheck them before relying on current information.`}</p></section>
