...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const copy = locale === "ar" ? normalizeArabicContent(careersCopy[locale]) : careersCopy[locale];
    ...
    <div className="careers-page" id="top">
      <main id="main-content">
        <PageHero description={copy.hero.text} eyebrow={copy.hero.eyebrow} image="careers" locale={locale} title={copy.hero.title}
          ...
          note={<><span>{isArabic ? "علياس العقارية / الوظائف" : "ARE / CAREERS"}</span><p>{copy.hero.note}</p></>} />
        ...
        <section aria-labelledby="career-areas-title" className="career-areas content-section">
          ...
          <ol>{careerInterests.map((interest, index) => <li key={interest.value}><span>{localizedDisplayText(String(index + 1).padStart(2, "0"), locale)}</span><h3>{interest.label[locale]}</h3><small>{isArabic ? "مجال اهتمام عام" : "General area of interest"}</small></li>)}</ol>
        </section>
        ...
        <section aria-labelledby="opportunities-title" className="career-opportunities">
          ...
          {careerVacancies.length === 0 ? <div className="career-opportunities__empty"><span aria-hidden="true">{isArabic ? "٠٠" : "00"}</span><div><h3>{copy.opportunities.emptyTitle}</h3><p>{copy.opportunities.emptyText}</p><Link className="button button--primary animated-gold-border" href="#application">{copy.opportunities.action}</Link></div></div> : <div className="career-vacancy-grid">{careerVacancies.map((job)=><article key={job.id}><span>{isArabic ? normalizeArabicContent(job.department) : job.department}</span><h3>{isArabic ? normalizeArabicContent(job.title) : job.title}</h3><p>{isArabic ? normalizeArabicContent(`${job.location} · ${job.employment_type}`) : `${job.location} · ${job.employment_type}`}</p><Link className="text-link" href={`/${locale}/careers/${job.slug}`}>{isArabic?"عرض الوظيفة":"View role"}</Link></article>)}</div>}
        </section>
