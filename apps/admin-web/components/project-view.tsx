...
export function ProjectView({ id, preview = false }: Readonly<{ id: string; preview?: boolean }>) {
  ...
  return <section>
    <AdminPageHeader back={<GuardedLink className="back-link" href="/projects"><ArrowLeft aria-hidden size={16}/>Back to Projects</GuardedLink>} eyebrow={preview ? "Private preview" : "Project record"} title={en.official_name ?? record.slug ?? "Project"} description={preview ? "This authenticated preview excludes internal source history and ARE Priority." : "Read-only canonical Project details."} action={<div className="status-cluster"><StatusBadge status={record.status ?? "draft"}/><StatusBadge status={record.workflow_status ?? "draft"}/></div>}/>
    ...
    <div className="sticky-actions"><GuardedLink className="primary-button" href={`/projects/${id}`}><Edit3 aria-hidden size={16}/>Edit Project</GuardedLink>{record.status === "published" ? <GuardedLink className="secondary-button" href={`/projects/${id}/revisions`}>Revision workflow</GuardedLink> : null}{preview ? <GuardedLink className="secondary-button" href={`/projects/${id}/view`}>View internal record</GuardedLink> : <GuardedLink className="secondary-button" href={`/projects/${id}/preview`}>Preview public-safe fields</GuardedLink>}</div>
  </section>;
}
