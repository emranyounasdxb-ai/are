...
export function ProjectView({ id, preview = false }: Readonly<{ id: string; preview?: boolean }>) {
  ...
  return <section><AdminPageHeader back={<GuardedLink className="back-link" href="/projects"><ArrowLeft aria-hidden size={16}/>Back to Projects</GuardedLink>} eyebrow={preview ? "Private preview" : "Project record"} title={en.official_name ?? record.slug ?? "Project"} description={preview ? "This authenticated preview excludes internal source history and ARE Priority." : "Read-only canonical Project details."} action={<div className="status-cluster"><StatusBadge status={record.status ?? "draft"}/><StatusBadge status={record.workflow_status ?? "draft"}/></div>}/><div className="detail-grid"><Detail title="Developer" value={record.developer?.slug}/><Detail title="Area" value={record.area?.name_en}/><Detail title="Availability" value={record.availability_status}/><Detail title="Construction" value={record.construction_status}/><Detail title="Handover" value={[record.handover_quarter, record.handover_year].filter(Boolean).join(" ") || record.original_handover_value}/><Detail title="Property types" value={record.property_types?.join(", ")}/><Detail title="Unit types" value={record.unit_types?.map((item) => item.label_en).filter(Boolean).join(", ")}/><Detail title="Bedrooms" value={record.bedroom_options?.join(", ")}/><Detail title="Size range" value={record.size_min || record.size_max ? `${record.size_min ?? "—"}–${record.size_max ?? "—"} ${record.size_unit ?? ""}` : undefined}/><Detail title="Down payment" value={record.down_payment_percentage == null ? undefined : `${record.down_payment_percentage}%`}/><Detail title="Amenities" value={record.amenities?.map((item) => item.label_en).filter(Boolean).join(", ")}/><Detail title="Nearby places" value={record.nearby_places?.map((item) => item.name_en).filter(Boolean).join(", ")}/><Detail title="Payment milestones" value={record.payment_plan?.milestones?.length ? String(record.payment_plan.milestones.length) : undefined}/><Detail title="Gallery and floor plans" value={record.media?.length ? `${record.media.length} media records` : undefined}/><Detail title="Last Verified" value={record.last_verified_at ? new Date(record.last_verified_at).toLocaleString("en-AE") : undefined}/>{preview ? null : <><Detail title="ARE Priority" value={record.priority ?? undefined}/><Detail title="Internal sources" value={record.sources?.length ? `${record.sources.length} source-history records` : undefined}/></>}</div><article className="content-block"><h2>Overview</h2><p>{en.short_summary || "No English summary supplied."}</p>{en.full_description ? <p>{en.full_description}</p> : null}</article><div className="sticky-actions"><GuardedLink className="primary-button" href={`/projects/${id}`}><Edit3 aria-hidden size={16}/>Edit Project</GuardedLink>{preview ? <GuardedLink className="secondary-button" href={`/projects/${id}/view`}>View internal record</GuardedLink> : <GuardedLink className="secondary-button" href={`/projects/${id}/preview`}>Preview public-safe fields</GuardedLink>}</div></section>;
}
