...
import { Archive, ArrowLeft, CheckCircle2, Eye, Plus, Save, Send, Trash2, Upload } from "lucide-react";
...
const initialValues = (): Values => ({ slug: "", developer_id: "", area_id: "", status: "draft", workflow_status: "draft", availability_status: "coming-soon", construction_status: "not-confirmed", handover_quarter: "", handover_year: "", original_handover_value: "", size_min: "", size_max: "", size_unit: "", down_payment_percentage: "", down_payment_source_value: "", latitude: "", longitude: "", last_verified_at: "", priority: "", featured: false, display_order: 0, internal_notes: "", property_types: [], bedroom_options: [], unit_types: [], amenities: [], nearby_places: [], translations: { en: emptyTranslation(), ar: emptyTranslation() }, sources: [], payment_raw: "", payment_source_index: 0, payment_complete: false, payment_verified_at: "", milestones: [], media: [] });
export function ProjectEditor({ id }: Readonly<{
    ...
    async function transition(action: "submit-review" | "approve", message: string) { if (!id)
...
function recordValues(record: ProjectRecord): Values {
    ...
    return {
        ...
        sources: record.sources?.map((item) => ({ ...item, retrieved_at: localDateTime(item.retrieved_at), last_checked_at: localDateTime(item.last_checked_at) })) ?? [],
        media: record.media?.map((item) => ({ ...item, verified_at: localDateTime(item.verified_at) })) ?? [],
