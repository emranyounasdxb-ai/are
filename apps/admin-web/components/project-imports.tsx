...
type Candidate = {
  ...
  arabic_review_required: boolean;
  acquisition_summary: Record<string, unknown>;
  evidence: unknown[];
  staged_media: unknown[];
  changes: unknown[];
};
...
export function ProjectImportDetail({ id }: Readonly<{ id: string }>) {
  ...
  return <section><AdminPageHeader back={<GuardedLink className="back-link" href="/project-imports"><ArrowLeft aria-hidden size={16}/>Back to Project Imports</GuardedLink>} description={batch.source_reference} eyebrow="Import Review Queue" title={batch.name}/><div className="status-summary" aria-label="Batch summary"><Summary label="Total" value={batch.total_count}/><Summary label="Ready for approval" value={batch.clean_count}/><Summary label="Needs review" value={batch.needs_review_count}/><Summary label="Failed" value={batch.failed_count}/></div>{review.error ? <InlineFeedback tone="error">{review.error.message}</InlineFeedback> : null}{!batch.candidates.length ? <DataTableShell label="Import candidates"><EmptyState description="This batch contains no staged candidates." title="No candidates"/></DataTableShell> : <div className="import-candidate-list">{batch.candidates.map((candidate) => <article className="panel import-candidate" key={candidate.id}><header><div><p className="eyebrow">Manifest row {candidate.owner_manifest_values.row_id as number}</p><h2>{candidateName(candidate)}</h2></div><StatusBadge status={candidate.review_status}/></header><dl className="detail-grid"><Detail label="Normalized name" value={candidate.normalized_project_name ?? "Unresolved"}/><Detail label="Proposed Developer" value={candidate.proposed_developer_id ?? "Unresolved"}/><Detail label="Proposed Area" value={candidate.proposed_area_id ?? "Unresolved"}/><SourceLink value={candidate.official_source_url}/><Detail label="Content hash" value={candidate.content_hash}/><Detail label="Last verified" value={date(candidate.last_verified_at)}/><Detail label="Source URLs" value={candidate.source_urls.join(", ") || "None"}/><Detail label="Arabic content" value={candidate.arabic_review_required ? "Review required" : "Official Arabic evidence found"}/><Detail label="Match result" value={json(candidate.match_result)}/><Detail label="Validation errors" value={json(candidate.validation_errors)}/><Detail label="Conflict reasons" value={json(candidate.conflict_reasons)}/><Detail label="Media references" value={`${candidate.staged_media.length} — rights remain Pending`}/></dl><details><summary>Owner manifest values</summary><pre>{json(candidate.owner_manifest_values)}</pre></details><details><summary>Extracted and normalized proposal</summary><pre>{json(candidate.normalized_payload)}</pre></details><details><summary>Acquisition outcome</summary><pre>{json(candidate.acquisition_summary)}</pre></details><details><summary>Official-source evidence metadata</summary><pre>{json(candidate.evidence)}</pre></details><details><summary>Staged media references</summary><pre>{json(candidate.staged_media)}</pre></details><details><summary>Refresh history</summary><pre>{json(candidate.changes)}</pre></details><details><summary>Bounded extracted payload (authenticated review only)</summary><pre>{json(candidate.raw_source_payload)}</pre></details><div className="row-actions"><button disabled={review.isPending} onClick={() => review.mutate({ candidateId: candidate.id, status: "ready-for-approval" })} type="button"><ListChecks aria-hidden size={15}/>Ready for approval</button><button disabled={review.isPending} onClick={() => review.mutate({ candidateId: candidate.id, status: "approved" })} type="button"><Check aria-hidden size={15}/>Approve</button><button disabled={review.isPending} onClick={() => review.mutate({ candidateId: candidate.id, status: "rejected" })} type="button"><CircleX aria-hidden size={15}/>Reject</button>{candidate.linked_project_id ? <button disabled={review.isPending} onClick={() => review.mutate({ candidateId: candidate.id, status: "merged" })} type="button"><GitMerge aria-hidden size={15}/>Mark merged</button> : null}</div></article>)}</div>}</section>;
}
...
function SourceLink({ value }: Readonly<{ value?: string | null }>) { return <div><dt>Official source</dt><dd>{value ? <a href={value} rel="noreferrer" target="_blank">Open official source</a> : "Unresolved"}</dd></div>; }
