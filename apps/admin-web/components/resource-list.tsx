...
export function ResourceList({ resource, title, newHref }: Readonly<{ resource: "properties" | "insights" | "jobs"; title: string; newHref: string }>) {
  ...
  const listState = resolveResourceListState(allQuery.data?.meta.total, query.data?.items.length, filtered);
  ...
  const createAction = <Link className="primary-button" href={newHref}><Plus aria-hidden size={16}/>{itemConfig.action}</Link>;
  const counts = statuses.map((value) => [value, allQuery.data?.items.filter((item) => item.status === value).length ?? 0] as const);
  return <section><AdminPageHeader action={hasRecords ? createAction : undefined} description={itemConfig.description} eyebrow="Content operations" title={title}/>
